#!/usr/bin/env python3
# agent_plus_fixed.py — patched version (debug + robust sentiment + expanded keywords)

import os, re, sqlite3, imaplib, smtplib, email, time, random, string, json
...
# Expanded review keywords (was updated)
REVIEW_KEYWORDS = [
    r"\breview\b", r"\bfeedback\b", r"\bcomplaint\b", r"\bpraise\b",
    r"\bexperience\b", r"\bservice\b", r"\bbad\b", r"\bterrible\b",
    r"\bawful\b", r"\bcold\b", r"\brude\b", r"\bwait\b", r"\bwaited\b",
    r"\bnever again\b", r"\bdisappoint\b", r"\brefund\b", r"\bworst\b"
]
...
def analyze_sentiment_with_backoff(message_text: str):
        ...
        for attempt in range(4):
            try:
                ...
                try:
                    ...
                    # sometimes model returns plain "negative, 4" — attempt to parse
                    m = re.search(r"(positive|neutral|negative).*?(\d)", txt, flags=re.I)
                    if m:
                        s = m.group(1).lower()
                        sc = int(m.group(2))
                    else:
                ...
                if s not in {"positive","neutral","negative"}:
                    s = "neutral"
                print(f"[LLM] sentiment -> {s}, score -> {sc}")
                return s, sc
            except Exception as e:
                wait = 2 ** attempt
    ...
    neg_keywords = ["awful","terrible","horrible","disgusting","cold","late","rude","bad","worst","never again","refund","angry","disappoint","hate","never"]
    pos_keywords = ["amazing","great","excellent","love","loved","fantastic","wonderful","perfect","delicious","thank","thanks"]
    neg_hits = sum(w in t for w in neg_keywords)
    pos_hits = sum(w in t for w in pos_keywords)
...
def handle_feedback(conn, from_email_addr, name, subject, body_text, in_reply_to):
    ...
    # debug
    print(f"[DEBUG] Feedback handler: sentiment={sentiment}, score={score}, discount={discount}, code={code}")
    # 5) Send
...
def handle_message(conn, raw):
    ...
    print(f"[DEBUG] Email intent classified as: {label} -> {from_email_addr}")
